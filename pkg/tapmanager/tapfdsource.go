...
func (s *TapFDSource) getDummyNetwork(podName, podNS string) (*cnicurrent.Result, string, error) {
	if s.dummyNetwork == nil {
		var err error
		s.dummyNetwork, s.dummyNetworkNsPath, err = s.cniClient.GetDummyNetwork(podName, podNS)
		if err != nil {
			return nil, "", err
		}
...
func (s *TapFDSource) GetFDs(key string, data []byte) ([]int, []byte, error) {
	...
	if err := s.setupNetNS(key, pnd, func(netNSPath string, allLinks []netlink.Link, hostNS ns.NetNS) (*network.ContainerSideNetwork, error) {
		...
		if err := nettools.FixCalicoNetworking(netConfig, s.calicoSubnetSize, s.getDummyNetwork, pnd.PodName, pnd.PodNs); err != nil {
