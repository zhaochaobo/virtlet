...
// namespace.
func FixCalicoNetworking(netConfig *cnicurrent.Result, calicoSubnetSize int, getDummyNetwork func(string, string) (*cnicurrent.Result, string, error),podName string, podNs string) error {
	for n, ipConfig := range netConfig.IPs {
		...
		if haveCalicoGateway {
			dummyNetwork, nsPath, err := getDummyNetwork(podName,podNs)
			if err != nil {
				return err
			}
